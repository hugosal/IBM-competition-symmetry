...
set.seed(26)
time_limits <- c(1, 2, 3, 5, 10, 20, 30, 40, 50)
res_acc_days <- data.frame(acc_sym = numeric(length(time_limits)),
           ...
           reg_tree_acc = numeric(length(time_limits)),
...
for (t_lim in time_limits){
  ...
  ## 1) regression tree with mean and sd of no. competitors and coef. var. as predictors
  coef_var_tree_discrete <- tree(comp_symmetry_factor ~ coef_var + 
                                   mean_no_competitors + 
                                   sd_no_competitors,
                                 data = training_repetitions_at_time_t)
  
  
  tree_accuracy_this_rep <- sum(
    testing_repetitions$comp_symmetry_factor == predict(coef_var_tree_discrete, 
                                                        testing_repetitions, 
                                                        type="class")
    )/nrow(testing_repetitions)
  
  ## 2) regression tree using only increase of decrease from the previous time to this one
  ...
  res_acc_days[counter_days, 1] <- sum(testing_repetitions_at_time_t$comp_symmetry_factor == "Symmetric" & BF > 1 )/sum(BF > 1)*100
  
  res_acc_days[counter_days, 2] <- mean(BF[testing_repetitions_at_time_t$comp_symmetry_factor == "Symmetric" & BF > 1 ])
  
  res_acc_days[counter_days, 3] <- sum(testing_repetitions_at_time_t$comp_symmetry_factor == "Assymetric" & BF < 1)/sum(BF < 1)*100
  
  res_acc_days[counter_days, 4] <- mean(BF[testing_repetitions_at_time_t$comp_symmetry_factor == "Assymetric" & BF < 1])
  
  res_acc_days[counter_days, 5] <- tree_accuracy_this_rep * 100
  
  res_acc_days[counter_days, 6] <- tree_accuracy_this_rep_dcoef * 100
  
  counter_days <- counter_days + 1
}

print(xtable::xtable(res_acc_days[, -5]))

false_asym <- which(BF < 1 & 
                       testing_repetitions_at_time_t$comp_symmetry_factor!="Symmetric")
...
t0$var_will_increase <- as.factor(sapply(1:nrow(t0), fundelta_coef_treection(x){
