...
#   t(as.matrix(reg_asym_mult$coefficients)) %*% xxx})
# 
# n_asym <- nrow(reg_asym_mult$x)
# 
# sigmaasym2 <- 1/df_asym *( t( reg_asym_mult$y -  XX %*% reg_asym_mult$coefficients) %*% 
#                              (reg_asym_mult$y - XX %*% reg_asym_mult$coefficients) )
# 
# sigmaasym2 <- sigmaasym2 + apply(x_new, MARGIN = 1, 
#                                  function(xxx){
#                                    t(xxx) %*% solve(t(XX) %*% XX) %*% xxx
#                                  } )
# 
# ncp_asym <- (y_mean-mean_asym) * sqrt(n_asym)/sqrt(sigmaasym2)
# 
# likelihood_asymm <- dt(x = testing_repetitions_final$coef_var  , 
#                        df = df_asym, 
#                        ncp = ncp_asym) 
# 
# ## para sym 
...
#                          center = F)
# yy_mean <- mean(only_last$coef_var[only_last$comp_symmetry_factor=="Symmetric"])
# XXsym <- as.matrix(cbind(rep(1, sum(only_last$comp_symmetry_factor=="Symmetric")), 
#                       only_last[only_last$comp_symmetry_factor=="Symmetric", c(8 , 9) ]))
# 
# df_sym <- reg_sym_mult$df.residual
# 
# mean_sym <- x_new %*% reg_sym_mult$coefficients
# 
# n_sym <- nrow(only_last[only_last$comp_symmetry_factor=="Symmetric", ])
# 
# sigmasym2 <- reg_sym_mult$post.sd**2 + apply(x_new, MARGIN = 1, 
#                                                function(xxx){
#                                                  t(xxx) %*% solve(t(XXsym) %*% XXsym) %*% xxx
#                                                } )
# 
# ncp_sym <-( yy_mean - mean_sym) * sqrt(n_sym)/sqrt(sigmasym2)
# 
# likelihood_symm <- dt(x = testing_repetitions_final$coef_var, 
#                        df = df_sym, 
#                        ncp = ncp_sym) 
# 
...
x_range <-seq(from=0, to=, 4, length.out=100)
...
mean_asym <-  interval_assym$pred.y
  ...
  #(x_range-mean(only_last$t_potential_competitors[only_last$comp_symmetry_factor=="Assymetric"]))*reg_asym$post.coef[2]

 sd_asym <- interval_assym$pred.se
   #(reg_asym$post.coef.sd[1]**2) +
#   (((x_range-mean(only_last$t_potential_competitors[
...
plot(1, type="n", xlim=c(0,3), ylim=c(0, 0.5), xlab="Mean Comp.", 
     ylab="Final coefficient of variation")
...
plot(x = testing_repetitions_final$sd_no_competitors, 
...
#      col=heat.colors(8)[as.factor(round(t0$t_potential_competitors))])
# 
...
will_increase_tree <- tree(var_will_increase~coef_var+n_overcrowding_plants+
                             t_potential_competitors+total_biomass+
